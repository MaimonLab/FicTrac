...
void Remapper::apply(const IplImage *src, IplImage *dst)
{
	...
	if (dst->width!=_dstW || dst->height!=_dstH) {
		fprintf(stderr, "Remapper::apply: unexpected destination image size! Boo!\n");
		abort();
	}
	...
	if (src->depth != dst->depth) {
		fprintf(stderr, "Remapper::apply: image depth mismatch\n");
		return;
	}
	...
	case IPL_DEPTH_32F:
		...
		break;
	default:
		fprintf(stderr, "Remapper::apply: invalid data type\n");
		return;
	}
}
	...
	int srcStep, int dstStep)
{
	...
	if ((srcStep>0 && (_srcW*bpp) > srcStep)
		...
		fprintf(stderr, "Remapper::_apply: failed channel sanity check!\n");
		return;
	}
...
// 	case LANCZOS: cvInterp = cv::INTER_LANCZOS4; break;
	default:
		fprintf(stderr, "Remapper::_apply: invalid interp mode\n");
		return;
	}
	...
	default:
		fprintf(stderr, "Remapper::_apply: invalid data type\n");
		return;
	}
